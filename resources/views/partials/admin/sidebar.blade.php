<label for="sidebar-toggle" class="lg:hidden fixed top-4 left-4 z-50 bg-[#1E3A8A] text-white p-2 rounded-md cursor-pointer shadow-md">
    ☰
</label>
...
<aside
    class="fixed lg:static inset-y-0 left-0 w-64 bg-gradient-to-b from-[#1E3A8A] to-[#1E40AF] text-white flex flex-col p-5 space-y-8 shadow-xl
    ...
    <nav aria-label="Admin sidebar" class="flex flex-col gap-2 text-[15px]">
        ...
        <form method="POST" action="{{ route('logout') }}" class="mt-6 border-t border-white/20 pt-4">
            @csrf
            <button type="submit"
                class="sidebar-link w-full text-left flex items-center gap-2 px-4 py-2 rounded-lg hover:bg-red-600/80 transition">
                🚪 <span>Logout</span>
            </button>
        </form>
    </nav>
